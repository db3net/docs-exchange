...
location ~ /(shared|node_modules|icons|exchange/api-console) {
    resolver ${DNS_RESOLVERS} valid=15;
...
location /resources {
    resolver ${DNS_RESOLVERS} valid=15;
    proxy_set_header X-Forwarded-Host $http_host;
    proxy_set_header X-Forwarded-Path /resources;
    proxy_pass https://anypoint.mulesoft.com/exchange/portals/merchant-e-solutions-3/resources/;
}
...
${DNS_RESOLVERS}: A list of DNS resolver IP addresses or hostnames.
----
